...
Stack e monorepo leve descritos em **[architecture.md](./architecture.md)** (Bun workspaces, NestJS, Expo). Não se aplica ao estado atual deste repositório, que é só `docs/`.
...
- Tutorial inicialmente conforme texto versionado em `docs/clinical-protocols/` e `docs/product/` até haver decisão sobre conteúdo dinâmico exposto pela API.  
...
Protótipo navegável pode apoiar validações com stakeholders autorizados (contexto institucional de saúde); qualquer decisão oriunda desse ciclo atualiza **`docs/product/`** antes das alterações de código correspondentes.
...
O cliente mobile consome apenas contratos atualizados; a OpenAPI no servidor deve ser tratada como **fonte técnica de verdade**, salvo espelhos opcionais em `docs/contracts/` quando assim for decidido. Opcional codegen (`openapi-typescript-codegen`) ou compartilhamento de schemas (**Zod**) em `packages/`.
...
Fluxos ou logs que tratam dados pessoais (incluindo **saúde**): devem estar coerentes com **[privacy-and-lgpd.md](../product/privacy-and-lgpd.md)** antes de uso em ambientes autorizados com titulares reais.
...
|----------|---------------|
| OpenAPI oficial / snapshots marcados semver | Swagger runtime + opcional cópias `docs/contracts/` |
